...
{
    osMutexId lock_mtx = osMutexCreate(NULL);
...
LOCK_RESULT Lock(LOCK_HANDLE handle)
{
    ...
    {
        osMutexId lock_mtx = (osMutexId)handle;
//        Mutex* lock_mtx = (Mutex*)handle;
        if (osMutexWait(lock_mtx, osWaitForever) == osOK)
...
LOCK_RESULT Unlock(LOCK_HANDLE handle)
{
    ...
    else
    {
        osMutexId lock_mtx = (osMutexId)handle;
//        Mutex* lock_mtx = (Mutex*)handle;
        if (osMutexRelease(lock_mtx) == osOK)
