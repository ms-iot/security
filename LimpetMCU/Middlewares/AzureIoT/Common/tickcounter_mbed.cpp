...
TICK_COUNTER_HANDLE tickcounter_create(void)
{
    ...
    result->last_clock_value = osKernelSysTick();
//    result->last_clock_value = clock();
    result->current_ms = result->last_clock_value * portTICK_PERIOD_MS;
...
int tickcounter_get_current_ms(TICK_COUNTER_HANDLE tick_counter, uint64_t* current_ms)
{
    ...
    else
    {
        ...
        clock_t clock_value = osKernelSysTick();
//        clock_t clock_value = clock();

        tick_counter_instance->current_ms += (clock_value - tick_counter_instance->last_clock_value) * portTICK_PERIOD_MS;
