...
function GenerateSecureBootFiles([xml] $config)
{
    ...
    $objectFromFormat | Set-SecureBootUEFI -SignedFilePath "$IntDir\db.bin.p7" -OutputFilePath "$OutputDir\SetVariable_db.bin" | Out-Null
    ...
    $objectFromFormat | Set-SecureBootUEFI -SignedFilePath "$IntDir\kek.bin.p7" -OutputFilePath "$OutputDir\SetVariable_kek.bin" | Out-Null
    ...
    $objectFromFormat | Set-SecureBootUEFI -SignedFilePath "$IntDir\pk.bin.p7" -OutputFilePath "$OutputDir\SetVariable_pk.bin" | Out-Null
}
...
function New-IoTSecureBootPackage([string] $ConfigFileName)
{
    ...
    try
    {
        ...
        MakeCabSingle -config $config -PackageXml (get-item -path "$OutputDir\DeviceGuard.SecureBoot.pkg.xml")
    }
    finally
    {
        Pop-Location 
